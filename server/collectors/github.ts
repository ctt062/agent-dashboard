...
const INSTALL_HINT =
  'Install GitHub CLI and run `gh auth login`, then refresh. Agent Deck reads your contribution calendar via local gh auth.'

const AUTH_HINT =
  'Run `gh auth login`, then refresh. Agent Deck reads your contribution calendar via local gh auth.'
...
export function collectGithub(): GithubSnapshot {
  try {
    ...
    const missingGh =
      /ENOENT|not found|command not found/i.test(message) ||
    ...
    return {
      ...
      totalContributions: 0,
      days: [],
      hint: missingGh ? INSTALL_HINT : AUTH_HINT,
    }
  }
}
