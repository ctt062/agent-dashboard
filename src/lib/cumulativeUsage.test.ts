...
describe('chartDomainStart', () => {
  it('prefers Cursor billing cycle over Claude weekly start', () => {
    const start = chartDomainStart(
      [
        ...
        agent({
          ...
          usageReset: {
            ok: true,
            cycleStart: '2026-07-21T16:00:00.000Z',
            windows: [{ label: 'Weekly', at: null }],
          },
          daily: [],
        }),
      ],
  ...
  it('spans billing-cycle start through today and scales plan percent', () => {
    const agents: AgentShare[] = [
      ...
      agent({
        ...
        daily: [
          { date: '2026-07-28', primary: 2, primaryLabel: 'messages' },
          { date: '2026-07-29', primary: 2, primaryLabel: 'messages' },
        ],
      }),
      agent({
        id: 'codex',
    ...
    assert.equal(chart.points.at(-1)?.claude, 100)
